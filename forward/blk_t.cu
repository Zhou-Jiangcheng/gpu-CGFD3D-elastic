...
{
  // mpi mesg
  size_t siz_x = (nj * nk * fdx_nghosts) * num_of_vars;
  size_t siz_y = (ni * nk * fdy_nghosts) * num_of_vars;
      ...
      float *sbuff)
{
  ...
  // local pointer
  float *Txx   = wav->v5d + wav->Txx_pos;
  float *Tyy   = wav->v5d + wav->Tyy_pos;
  float *Tzz   = wav->v5d + wav->Tzz_pos;
  float *Txz   = wav->v5d + wav->Txz_pos;
  float *Tyz   = wav->v5d + wav->Tyz_pos;
  float *Txy   = wav->v5d + wav->Txy_pos;
            ...
            gdinfo_t *gdinfo, wav_t *wav, float *sbuff)
{
  ...
  // local pointer
  float *Vx    = wav->v5d + wav->Vx_pos ;
  float *Vy    = wav->v5d + wav->Vy_pos ;
  float *Vz    = wav->v5d + wav->Vz_pos ;
